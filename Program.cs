...
#region Unnecessary code but keeping in-case asked about
// Loading the tables isn't needed, we already execute the query commands when we want to.
/*
internal static SqlConnection SqlConnection = new("Data Source=ADAM;Initial Catalog=e-commerce-database;Integrated Security=True;Trust Server Certificate=True");
private static void LoadAdminTable()
{
    SqlConnection.Open();
    var cmd = new SqlCommand("SELECT Admin_ID, Admin_Name, Admin_Email, Phone_Number FROM dbo.Admin", SqlConnection);
...
{
    SqlConnection.Open();
    var cmd = new SqlCommand("SELECT Category_ID, Category_Name, Category_Description FROM dbo.Category", SqlConnection);
    cmd.CommandType = CommandType.Text;
...
private static void LoadSellerTable()
{
    SqlConnection.Open();
    var cmd =
        new SqlCommand("SELECT Seller_ID, Seller_Name, Seller_Email, Phone_Number, Seller_Address FROM dbo.Seller",
            SqlConnection);
    cmd.CommandType = CommandType.Text;
...
private static void LoadCustomerTable()
{
    ...
    var cmd =
        new SqlCommand(
            "SELECT Customer_ID, First_Name, Last_Name, Email, Phone_Number, Date_Of_Birth FROM dbo.Customer", SqlConnection);
    cmd.CommandType = CommandType.Text;
...
private static void LoadCustomerAddressTable()
{
    SqlConnection.Open();
    var cmd =
        new SqlCommand("SELECT Address_ID, Customer_ID, Label, City, Address_Description FROM dbo.Customer_Address",
            SqlConnection);
    cmd.CommandType = CommandType.Text;
...
private static void LoadPaymentTable()
{
    SqlConnection.Open();
    var cmd =
        new SqlCommand("SELECT Card_Number, CardHolder_Name, Card_Expiry_Date, Customer_ID FROM dbo.Payment", SqlConnection);
    cmd.CommandType = CommandType.Text;
...
private static void LoadProductTable()
{
    ...
    var cmd =
        new SqlCommand(
            "SELECT Product_ID, Product_Name, Price, Quantity_In_Stock, Category_ID, Seller_ID FROM dbo.Product", SqlConnection);
    cmd.CommandType = CommandType.Text;
...
private static void LoadOrderTable()
{
    ...
    var cmd =
        new SqlCommand(
            "SELECT Order_ID, Order_Date, Order_Status, Shipping_Address_ID, Billing_Address_ID, Customer_ID, Total_Price FROM dbo.[Order]",
            SqlConnection);
    cmd.CommandType = CommandType.Text;
...
{
    SqlConnection.Open();
    var cmd = new SqlCommand("SELECT Cart_ID, Customer_ID, Cart_Expiry_Date, Total_Price FROM dbo.Cart", SqlConnection);
    cmd.CommandType = CommandType.Text;
...
{
    SqlConnection.Open();
    var cmd = new SqlCommand("SELECT Product_ID, Admin_ID FROM dbo.Product_Manages", SqlConnection);
    cmd.CommandType = CommandType.Text;
...
{
    SqlConnection.Open();
    var cmd = new SqlCommand("SELECT Category_ID, Admin_ID FROM dbo.Category_Manages", SqlConnection);
    cmd.CommandType = CommandType.Text;
...
{
    SqlConnection.Open();
    var cmd = new SqlCommand("SELECT Order_ID, Product_ID, Quantity, Unit_Price FROM dbo.order_contains_product",
        SqlConnection);
    cmd.CommandType = CommandType.Text;
...
{
    SqlConnection.Open();
    var cmd = new SqlCommand("SELECT Cart_ID, Product_ID, Quantity FROM dbo.Added_To", SqlConnection);
    cmd.CommandType = CommandType.Text;
